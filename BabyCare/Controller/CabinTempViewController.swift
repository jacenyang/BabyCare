...
class CabinTempViewController: UIViewController {
    ...
    func setChartValues(_ count : Int = 20) {
        ...
        var i = 0
        for da in dataAPI{
            let d = Double("\(da.field1)")
            arr.append(ChartDataEntry(x: Double(i), y: d ?? 0.0))
            i+=1
        }
        print(arr)
        let set1 = LineChartDataSet(entries: arr, label: "Suhu Kabin")
...
extension CabinTempViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! CabinTempTableViewCell
        
        let cabinTempDouble = Double(dataAPI.reversed()[indexPath.row].field1)
        let cabinTempString = String(format: "%.1f", cabinTempDouble!) + " ℃"
        cell.numberLabel.text = cabinTempString
