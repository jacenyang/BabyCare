...
class HumidityViewController: UIViewController {
    ...
    func setChartValues(_ count : Int = 20) {
        ...
        var i = 0
        for da in dataAPI{
            let d = Double("\(da.field2)")
            arr.append(ChartDataEntry(x: Double(i), y: d ?? 0.0))
            i+=1
        }
        print(arr)
        let set1 = LineChartDataSet(entries: arr, label: "Kelembaban")
...
extension HumidityViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! HumidityTableViewCell
        
        let humidityDouble = Double(dataAPI.reversed()[indexPath.row].field2)
        let humidityString = String(format: "%.0f", humidityDouble!) + " %"
        cell.numberLabel.text = humidityString
